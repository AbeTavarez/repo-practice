...
class Alien {
  constructor(hp, fp, acc, sh) {
    ...
    this.alerts = {
      ...
      damaged: (attacker) =>
        `You breach its hull for ${attacker.firepower} damage, leaving ${this.currentHull} health.`, //2
  ...
  constructor(hp, fp, acc, sh) {
    ...
    this.alerts = {
      ...
      damaged: (attacker) =>
        `You breach its hull for ${attacker.firepower} damage, leaving ${this.currentHull} health.`, //2
      destroyed: () => `You breach its hull and destroy it!`, //3
      damagedShield: (attacker) =>
        `You erode its shield for ${attacker.firepower} damage, leaving ${this.currentShield} shield health.`, //4
...
class Uss extends Alien {
  constructor(hp, fp, acc) {
    ...
    this.alerts = {
      ...
      damaged: (attacker) =>
        `Your hull is breached for ${attacker.firepower} damage, leaving ${this.currentHull} health.`, //2
  ...
  attack(target) {
    target.loadinShield > 0
      ? alert(this.alerts["attackBoss"]())
      : alert(this.alerts["attack"]());
  }
  destroyed() {
    alert(this.alerts["destroyed"]());
  }
...
class GameLogic {
  constructor() {
    ...
    this.messages = {
      ...
      attackOrRetreat: () => `[Current Health: ${
        this.ussSchwarzenegger.currentHull
      }] [Target's Health: ${
        this.alienFleet[0].currentHull
      }] [Enemies Remaining: ${this.alienFleet.length}],
  ...
  alienAttackResult() {
    const ussDodge = () => {
      this.ussSchwarzenegger.dodgeAttack();
      this.randomEnemySneakAttack();
      this.ussCounterAttack();
    };
    const ussTakeDamage = () => {
      this.ussSchwarzenegger.takeDamage(this.alienFleet[0]);
      this.randomEnemySneakAttack();
      this.ussCounterAttack();
    };
    const ussDestroyed = () => {
      this.ussSchwarzenegger.destroyed();
      this.endGame();
    };
    this.ussSchwarzenegger.currentHull === this.ussSchwarzenegger.startingHull
      ? ussDodge()
      : this.ussSchwarzenegger.currentHull > 0
      ? ussTakeDamage()
      : ussDestroyed();
  }
  ussAttackResult() {
    const alienDodge = () => {
      this.alienFleet[0].dodgeAttack();
      this.enemyCounterAttack();
    };
    const alienTakeDamge = () => {
      this.alienFleet[0].takeDamage(this.ussSchwarzenegger);
      this.enemyCounterAttack();
    };
    const alienDestroyed = () => {
      this.alienFleet[0].destroyed();
      this.randomHealthBonus();
      this.checkIfBossDefeated();
    };
    this.alienFleet[0].startingShield > 0
      ? this.ussAttackResultShield()
      : this.alienFleet[0].currentHull === this.alienFleet[0].startingHull
      ? alienDodge()
      : this.alienFleet[0].currentHull > 0
      ? alienTakeDamge()
      : alienDestroyed();
  }
  ussAttackResultShield() {
    ...
    const shieldTakeDamage = () => {
      this.alienFleet[0].takeShieldDamage(this.ussSchwarzenegger);
      this.enemyCounterAttack();
    };
    const shieldDestroyed = () => {
      this.alienFleet[0].shieldDestroyed(this.ussSchwarzenegger);
      this.userChoice(
        "attackOrRetreatBoss",
        "attack",
        "retreat",
    ...
    this.alienBoss.currentShield === this.alienBoss.startingShield
      ? shieldDodge()
      : this.alienBoss.currentShield > 0
      ? shieldTakeDamage()
      : shieldDestroyed();
  }
  enemyCounterAttack() {
  ...
  enemySneakAttack() {
    ...
    const ussTakeDamageSA = () => {
      alert(this.ussSchwarzenegger.alerts["damaged"](this.alienFleet[1]));
      this.ussSchwarzenegger.updateStartingHall();
    };
    const ussDestroyedSA = () => {
      this.ussSchwarzenegger.destroyed();
      this.endGame();
    };
  ...
  spawnAliens() {
    for (let i = 0; i < this.userDifficulty; i++) {
      let alienShip = new Alien(
        this.randomAlienProps(3, 6),
        this.randomAlienProps(2, 4),
        this.randomAlienProps(6, 8),
        0
      );
  ...
  hitOrMiss(target, attacker) {
    target.startingShield > 0
      ? Math.random() <= attacker.accuracy
        ? (target.currentShield -= attacker.firepower)
        : (target.currentShield -= 0)
      : Math.random() <= attacker.accuracy
      ? (target.currentHull -= attacker.firepower)
  ...
  randomEnemySneakAttack() {
    Math.random() <= 0.4 && this.alienFleet.length > 1
      ? this.enemySneakAttack()
      : false;
  }
  randomHealthBonus() {
    this.alienFleet.length !== this.startingFleetSize
      ? Math.random() <= 0.7
        ? (this.ussSchwarzenegger.currentHull += 1) &&
          alert(this.ussSchwarzenegger.alerts["repaired"]())
        : (this.ussSchwarzenegger.currentHull += 0)
      : false;
  }
  //////* Messaging Methods////
  userChoice(property, choice1, choice2, property1, property2) {
    const outcomes = {
      ...
      retreatNo: () =>
        this.alienFleet[0].loadinShield > 0
          ? this.userChoice(
              "attackOrRetreatBoss",
              "attack",
              "retreat",
  ...
  changeMessages1() {
    return this.alienFleet.length === this.startingFleetSize
      ? `first`
      : this.alienFleet.length > 1
      ? `next`
      : `last`;
  }
